...
const { status, showProgress, syncError, syncCompleteCount, startPolling, stopPolling } =
  useLogbookSync()
...
const filteredDurationLabel = computed(() => {
  const hours = Math.floor(totalDurationMinutes.value / 60)
  const minutes = totalDurationMinutes.value % 60
  return formatDurationProse(`${hours}:${String(minutes).padStart(2, '0')}`)
})
...
async function onFiltersApply(payload: {
  ...
  const location = {
    name: 'flights',
    query: flightListStateToQuery(sortPreset.value, draftFilters.value),
  }
  if (router.resolve(location).fullPath !== route.fullPath) skipNextRouteReload = true
...
onMounted(async () => {
  ...
  void startPolling()
  await ensureLoaded()
  const routeState = flightListStateFromQuery(route.query)
  sortPreset.value = route.query.sort ? routeState.sortPreset : initialSortPreset()
  draftFilters.value = routeState.filters
  await reloadFlights()
  await nextTick()
  window.scrollTo({ top: 0, left: 0 })
...
<template>
  <div class="space-y-6">
    ...
    <ErrorBanner
      v-else-if="error && !flights.length && !showProgress"
      :message="error"
      :retry-busy="loading"
      @retry="reloadFlights"
    />
    ...
    >
      <template v-if="hasActiveFilters"> No flights match your filters. </template>
      <template v-else> No flights yet. Add your first flight to get started. </template>
    </div>

    <div v-else-if="listInitialized && (flights.length || showProgress)" class="space-y-3">
      <ErrorBanner
        v-if="error && flights.length"
        :message="error"
        :retry-busy="loadingMore"
      ...
      >
        ...
        <span class="text-amber-800"> · {{ filteredDurationLabel }}</span>
